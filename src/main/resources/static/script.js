...
function updateSensorDisplay(type, value, maxValue) {
    // Cập nhật giá trị
    const sensorElement = document.querySelector(`.sensor-circle.${type} .sensor-value`);
    const displayValue = type === 'light' ? Math.round(value) : value.toFixed(1);

    if (sensorElement) {
        // Add animation class for value change
        sensorElement.classList.add('value-updating');
        sensorElement.textContent = displayValue;

        // Remove animation class after animation completes
        setTimeout(() => {
            sensorElement.classList.remove('value-updating');
        }, 400);
    }
    ...
    if (sensorCircle) {
        // Set CSS custom properties for the circular progress
        sensorCircle.style.setProperty('--progress', `${percentage}%`);
        sensorCircle.style.setProperty('--progress-deg', `${(percentage / 100) * 360}deg`);

        // Add visual feedback based on value ranges
        sensorCircle.classList.remove('low-value', 'medium-value', 'high-value');

        if (type === 'temperature') {
            if (value >= 35) {
                sensorCircle.classList.add('high-value');
            } else if (value <= 15) {
                sensorCircle.classList.add('low-value');
            } else {
                sensorCircle.classList.add('medium-value');
            }
        ...
        sensorCircle.classList.add('progress-updating');
        setTimeout(() => {
            sensorCircle.classList.remove('progress-updating');
        }, 800);
    }
}
